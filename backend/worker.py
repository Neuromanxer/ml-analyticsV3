...
def do_classification_predict(
    ...
    """
    Make predictions on new data using a previously trained classification model.
...
def do_segment_analysis(
    ...
    """Main segmentation analysis function"""
    
    try:  # Add try-catch wrapper like in clustering
        # ───────────── Directory prep ─────────────
        user_dir = PathL("static/images") / str(user_id)
        user_dir.mkdir(parents=True, exist_ok=True)
        PathL("models").mkdir(exist_ok=True)
        PathL("data").mkdir(exist_ok=True)

        # ───────────── Validate upload mode (same as clustering) ─────────────
        if file_path and (train_path or test_path):
            raise ValueError("Provide either file_path or both train_path+test_path, not both.")
        if (train_path and not test_path) or (test_path and not train_path):
            raise ValueError("Both train_path and test_path must be provided together.")
        ...
        # ───────────── Load data (match clustering logic) ─────────────
        if file_path:
            # Single file mode
            df = pd.read_csv(file_path)
            dataset_name = os.path.basename(file_path)
        ...
        for stat_func in ["mean", "median", "std", "min", "max"]:
            try:
                ...
                print(f"Skipped aggregation '{stat_func}' due to missing columns: {e}")
        ...
        # ───────────── Save data and model ─────────────
        segmented_data_path = PathL("data") / f"{user_id}_segmented_data.csv"
        df.to_csv(segmented_data_path, index=False)

        model_path = PathL("models") / f"{user_id}_kmeans_model.pkl"
        scaler_path = PathL("models") / f"{user_id}_scaler.pkl"
...
def do_label_clusters(
    ...
    try:
        ...
        # Validate upload mode (same structure as classification)
        if file_path and (train_path or test_path):
            raise ValueError("Provide either file_path or both train_path+test_path, not both.")
        if (train_path and not test_path) or (test_path and not train_path):
            raise ValueError("Both train_path and test_path must be provided together.")
        ...
        if file_path:
            ...
            if not clustered_data_path.exists():
                clustered_data_path = PathL("data") / f"{user_id}_segmented_data.csv"
                if not clustered_data_path.exists():
                    return {"status": "error", "message": f"No clustering data found for user ID: {user_id}"}
            df = pd.read_csv(clustered_data_path)
            dataset_name = os.path.basename(file_path)
        else:
            # Even for train/test mode we operate on previously clustered data
            clustered_data_path = PathL("data") / f"{user_id}_clustered_data.csv"
            if not clustered_data_path.exists():
                clustered_data_path = PathL("data") / f"{user_id}_segmented_data.csv"
                if not clustered_data_path.exists():
        ...
        # Save labeled data
        labeled_data_path = PathL("data") / f"{user_id}_labeled_data.csv"
        ...
        try:
            ...
            entry["segment_stats"] = segment_stats
            entry["segment_insights"] = segment_insights
            _append_limited_metadata(user_id, entry, max_entries=5) 

        except Exception as meta_err:
            logger.error(f"Metadata save error: {meta_err}")
        ...
        try:
            ...
            with open(request_json, "w") as f:
                json.dump({
                    "user_id": str(user_id),
                    "model_path": str(model_path.resolve()),
                    "data_path": str(processed_data_path.resolve()),  # Assuming you meant this to be data_path.resolve()
                    "output_dir": str(user_dir.resolve()),
            ...
            try:
                ...
                meta_list = json.loads(meta_file.read_text(encoding="utf-8")) if meta_file.exists() else []
            ...
            try:
                insights = generate_insights("regression", response_data)
                response_data["insights"] = insights
            except Exception as insight_err:
                logger.warning(f"Insight generation failed: {insight_err}")
                response_data["insights"] = "Could not generate insights."
...
def do_regression_predict(
    ...
    try:
        ...
        if not model_path.exists() or not preprocessor_path.exists():
            raise ValueError("No trained model or preprocessor found for this user")
        ...
        # Save output file with predictions
        output_path = user_dir / f"predictions_{PathL(file_path).stem}.csv"
        output_df.to_csv(output_path, index=False)
        print(f"[💾] Predictions saved to {output_path}")

        # ───────────── Visualization subprocess (optional) ─────────────
        request_json = user_dir / "prediction_request.json"
        with open(request_json, "w") as f:
            json.dump({
                "user_id": str(user_id),
                "data_path": str(output_path.resolve()),
                "output_dir": str(user_dir.resolve()),
        ...
        try:
            subprocess.run(
                ["python3", str(PathL("prediction_viz_runner.py").resolve()), str(request_json.resolve())],
                check=True
            )
            with open(user_dir / "prediction_result.json") as f:
                result = json.load(f)
            imp_df = pd.DataFrame(result.get("imp_df"))
            prediction_dist_img = result.get("prediction_distribution")
            feature_relationships = result.get("feature_relationships", {})
        except Exception as viz_error:
            print(f"[⚠️] Visualization error: {viz_error}")
            prediction_dist_img, feature_relationships = "", {}
        ...
        try:
            ...
            if prediction_dist_img:
                entry["visualizations"]["prediction_distribution"] = f"data:image/png;base64,{prediction_dist_img}"

            if feature_relationships:
                entry["visualizations"]["feature_relationships"] = {
                    k: f"data:image/png;base64,{v}" for k, v in feature_relationships.items()
                }

            _append_limited_metadata(user_id, entry, max_entries=5) 
            print(f"[✅] Metadata saved for prediction")
        ...
        response_data = {
            ...
            "processed_data_shape": df_processed.shape,
            "model_info": {
                "model_type": type(model).__name__,
                "feature_count": X_transformed.shape[1] if hasattr(X_transformed, 'shape') else None
            },
            "metrics": pred_stats,
            "visualizations": {},
            "insights": insights
        ...
        if prediction_dist_img:
            response_data["visualizations"]["prediction_distribution"] = f"data:image/png;base64,{prediction_dist_img}"

        if feature_relationships:
            response_data["visualizations"]["feature_relationships"] = {
                k: f"data:image/png;base64,{v}" for k, v in feature_relationships.items()
            }
...
def do_segment_analysis(
    ...
    Args:
        ...
        file_path: Path to single CSV file (optional)
        train_path: Path to training CSV file (optional)
        test_path: Path to test CSV file (optional)
        target_column: Column to exclude from clustering
    ...
    columns_to_drop = []
    if drop_columns:
        columns_to_drop.extend([col.strip() for col in drop_columns.split(",") if col.strip()])
    ...
    # Prepare data for clustering
    X = df.drop(columns=columns_to_drop) if columns_to_drop else df
    ...
    max_k = min(10, len(df) - 1)
    if max_k < 2:
        raise ValueError("Not enough data points for clustering (minimum 2 required)")
    
    best_k, distortions, silhouette_scores = find_optimal_k(X_scaled, max_k)

    # ───────────── Run KMeans ─────────────
    clusters, kmeans = run_kmeans(X_scaled, best_k)
    df["cluster"] = clusters
    ...
    plt.plot(K_range, distortions, 'bx-', linewidth=2, markersize=8)
    ...
    # ───────────── Segment Counts ─────────────
    segment_counts = df["cluster"].value_counts().sort_index().reset_index()
    ...
    try:
        ...
        entry = {
            ...
            "thumbnailData": f"data:image/png;base64,{cluster_viz_base64}" if cluster_viz_base64 else f"data:image/png;base64,{elbow_base64}",
            "imageData": f"data:image/png;base64,{cluster_viz_base64}" if cluster_viz_base64 else f"data:image/png;base64,{elbow_base64}",
            "visualizations": {
                "cluster_plot": f"data:image/png;base64,{cluster_viz_base64}" if cluster_viz_base64 else "",
                "elbow_method": f"data:image/png;base64,{elbow_base64}" if elbow_base64 else ""
            },
        ...
        logger.warning(f"Metadata save error: {meta_error}")
    ...
    # Add visualizations safely
    if cluster_viz_base64:
        response_data["visualizations"]["cluster_plot"] = f"data:image/png;base64,{cluster_viz_base64}"
    ...
    try:
        ...
        logger.warning(f"Insight generation failed: {insight_err}")
        response_data["insights"] = {"error": "Could not generate insights"}
...
def do_visualization(
    ...
    fig1, ax = plt.subplots(figsize=(8, 5))  # ✅ correctly assign axis
    ...
    if feature_column in NUMS:
        try:
            model_path = PathL(f"models/{user_id}_best_classifier.pkl")
            if not model_path.exists():
                model_path = PathL(f"models/{user_id}_best_regressor.pkl")
        ...
        try:
            ...
            meta_file.write_text(json.dumps(meta_list, indent=2), encoding="utf-8")
...
def do_counterfactual(
    ...
    try:
        ...
        entry = {
            ...
            "visualizations": {
                f"counterfactual_{i+1}": f"data:image/png;base64,{viz}" for i, viz in enumerate(visualizations)
            },
            "counterfactuals": counterfactuals_data,
            "metrics": sample_metrics
        }
        ...
        print(f"Metadata save error: {meta_err}")
...
def do_survival(
    ...
    try:
        ...
        model_path = PathL("models") / f"{user_id}_survival_cox.pkl"
        joblib.dump(cph_full, model_path)
...
def do_what_if(
    ...
    try:
        ...
        # Load user model
        model_path = f"models/{user_id}_best_classifier.pkl"
        if not os.path.exists(model_path):
            model_path = f"models/{user_id}_best_regressor.pkl"
            if not os.path.exists(model_path):
...
def do_decision_paths(
    ...
    metrics_headers = ["Metric", "Value"]
        ...
        # Convert summary_stats to a list of dictionaries (records)
    summary_stats = [
        {
            "Feature": col,
    ...
    # Optionally split into lines for easier frontend rendering
    ...
    try:
        ...
        print(f"Metadata save error: {meta_error}")
    
    try:
        insights = generate_insights("decision_paths", response_data)
        response_data["insights"] = insights
    except Exception as insight_err:
        print(f"Insight generation failed: {insight_err}")
        response_data["insights"] = "Could not generate insights."
